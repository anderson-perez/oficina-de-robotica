...
class Robo {
  ...
  private:
    ...
    unsigned int port_vel_m1;     // Velocidade motor da esquerda
    unsigned int port_vel_m2;     // Velocidade motor da direita
  ...
  public:
    void setup(unsigned int portM11, 
           ...
           unsigned int portVEL1,
           unsigned int portVEL2,
           ...
           )
{
  ...
  port_m2_1     = portM21;
  port_m2_2     = portM22;
  port_vel_m1   = portVEL1;
  port_vel_m2   = portVEL2;
  ...
  pinMode(port_vel_m1, OUTPUT);
  pinMode(port_vel_m2, OUTPUT);
...
{
  analogWrite(port_vel_m1, velocidade);
  analogWrite(port_vel_m2, velocidade);
...
void Robo::ir_para_tras(unsigned int velocidade)
{
  ...
  digitalWrite(port_m1_2, HIGH);
  digitalWrite(port_m2_2, HIGH);
}
...
{
  analogWrite(port_vel_m1, 0);
  analogWrite(port_vel_m2, 0);  
}
